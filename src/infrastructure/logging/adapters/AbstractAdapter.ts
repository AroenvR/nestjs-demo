...
import { ILogger, TLogMetadata } from '../ILogger';
import { ICorrelationManager } from '../correlation/ICorrelationManager';
import { ILoggerConfig, TLogLevels } from '../ILoggerConfig';
...
export abstract class AbstractAdapter<T> implements ILogger {
	...
	public verbose(message: string, metadata?: TLogMetadata): void {
		this.handle('verbose', message, metadata);
	}

	public debug(message: string, metadata?: TLogMetadata): void {
	...
	public info(message: string, metadata?: TLogMetadata): void {
		if (this.isWhitelistApproved(message)) this.handle('info', message, metadata);
		else this.handle('verbose', `Unwhitelisted message: ${message}`);
	}

	public log(message: string, metadata?: TLogMetadata): void {
	...
	public warn(message: string, metadata?: TLogMetadata): void {
		if (this.isWhitelistApproved(message)) this.handle('warn', message, metadata);
		else this.handle('verbose', `Unwhitelisted message: ${message}`);
	}

	public error(message: string, metadata?: TLogMetadata): void {
		if (this.isWhitelistApproved(message)) this.handle('error', message, metadata);
		else this.handle('verbose', `Unwhitelisted message: ${message}`);
	}

	public critical(message: string, metadata?: TLogMetadata): void {
	 ...
	 */
	protected abstract handle(level: TLogLevels, message: string, metadata?: TLogMetadata): void;
