...
describe(TEST_NAME, () => {
	...
	beforeAll(async () => {
		app = await createMockAppModule(undefined, true);
	...
	it("Can send a GET request using SSL to the WWW", async () => {
		...
		adapter = new TestApiAdapter(mockWinstonAdapter, requestBuilder, new MockConfigService(externalConfig), eventConsumer);
