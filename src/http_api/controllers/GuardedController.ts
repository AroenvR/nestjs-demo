import { BadRequestException, Body, HttpException, HttpStatus, Param, ParseIntPipe, Query, UseFilters, UseGuards } from '@nestjs/common';
...
export class GuardedController {
     ...
     */
    public async findAll(@Query() query: QueryDto): Promise<ResponseDto[]> {
        if (!isTruthy(query)) {
            this.logger.info(`Finding all entities`);
            return this.service.findAll();
        }

        this.logger.info(`Finding all entities by query parameter`);
        return this.service.findAllByQuery(query);
    }
